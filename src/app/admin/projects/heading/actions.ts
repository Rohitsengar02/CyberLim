...
export async function updateProjectHeadingConfig(prevState: any, formData: FormData) {
  try {
    ...
    const db = getDb();
    const docRef = db.collection("projectHead").doc("main");
    await docRef.set(parsed.data, { merge: true });

    revalidatePath("/");
    revalidatePath("/admin/projects/heading");

    return { success: true, message: "Project heading content updated successfully!" };
  } catch (error) {
    console.error("Error updating project heading config:", error);
