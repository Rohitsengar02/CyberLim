...
const projectsSections = [
  {
    ...
    href: "/admin/projects/heading",
  },
  {
    title: "Manage Projects",
    description: "Add, edit, delete, and manage layout for all project cards.",
    icon: <GalleryHorizontal className="h-6 w-6 text-accent" />,
    href: "/admin/projects/cards",
    disabled: false,
  },
  {
    title: "Animation Settings",
    description: "Adjust animation properties of the section (Coming Soon).",
...
export default function ProjectsAdminPage() {
    ...
    useGSAP(() => {
        gsap.fromTo('.project-admin-card', {
        opacity: 0,
        y: 50,
        scale: 0.95
        }, {
  ...
  return (
    <div ref={containerRef} className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {projectsSections.map((section) => (
        <Card key={section.title} className="project-admin-card bg-secondary/30 backdrop-blur-lg border-white/10 transition-all duration-300 ease-in-out hover:scale-[1.03] hover:bg-secondary/50 hover:border-white/20 hover:shadow-xl hover:shadow-accent/10 flex flex-col">
