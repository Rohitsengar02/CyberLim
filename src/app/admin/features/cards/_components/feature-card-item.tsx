...
import { deleteFeatureCard } from '../actions';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function FeatureCardItem({ card }: { card: FeatureCard }) {
  const { toast } = useToast();
  const [state, formAction, isPending] = useActionState(deleteFeatureCard, { success: false, message: "" });
    ...
    <Card className="bg-secondary/30 backdrop-blur-lg border-white/10 flex flex-col">
      ...
      <CardFooter className="p-3 bg-secondary/50 border-t border-border flex justify-end gap-2">
        <Button asChild variant="outline" size="icon">
          <Link href={`/admin/features/cards/edit?id=${card.id}`}>
        ...
        <AlertDialog>
          ...
          <AlertDialogContent className="bg-secondary/50 backdrop-blur-lg border-border">
             <form action={formAction}>
                ...
                <AlertDialogDescription>
                    This action cannot be undone. This will permanently delete the feature card.
                </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
