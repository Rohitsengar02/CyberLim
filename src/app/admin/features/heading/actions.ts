...
export async function updateFeaturesContentConfig(prevState: any, formData: FormData) {
  try {
    ...
    const db = getDb();
    const docRef = db.collection("featuresContent").doc("main");
    await docRef.set(parsed.data, { merge: true });

    revalidatePath("/");
    revalidatePath("/admin/features/heading");

    return { success: true, message: "Features content updated successfully!" };
  } catch (error) {
    console.error("Error updating features content config:", error);
